let textarea = document.getElementById('text_news');
	...
	async function load_logs( page_limit ) {
		...
		if ( result.error ) {
			...
			if ( result.page > 0 ) page_num += '<div class="pad_pagin"><button class="btn btn-primary" onclick="load_logs('+page_num_prev+')">Вперёд</button></div>';
			if ( result.pages > 1 ) page_num += '<div class="pad_pagin"><button class="btn btn-primary">'+current_page+'</button></div>'; 
			if ( page_num_next < result.pages ) page_num += '<div class="pad_pagin"><button class="btn btn-primary" onclick="load_logs('+page_num_next+')">Назад</button></div>';						
